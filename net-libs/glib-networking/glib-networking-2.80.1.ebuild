...
IUSE="+gnome +libproxy +ssl gnutls test"
RESTRICT="!test? ( test )"
...
RDEPEND="
	...
	ssl? (
		...
		!gnutls? ( >=dev-libs/openssl-1.0.2[${MULTILIB_USEDEP}] )
	)
...
multilib_src_configure() {
	local emesonargs=(
        ...
        if use ssl; then
		if use gnutls; then
			emesonargs+=( -Dgnutls=enabled )
			emesonargs+=( -Dopenssl=disabled )
		else
			emesonargs+=( -Dgnutls=disabled )
			emesonargs+=( -Dopenssl=enabled )
		fi
	fi
